...
@Component({
    selector: 'jhi-flowers-low-market-price-list',
    templateUrl: './low-market-price-list.component.html',
    styleUrls: ['./price-list.scss'],
    providers: [PriceListService, PriceListTranslationService]
})

export class LowMarketPriceListComponent implements OnInit, OnDestroy {
    ...
    loadAll() {
        ...
        const fillAllMarketVarietyPropertyByType = this.priceListService.getMarketVarietyPropertiesByType(this.low);
        const fillPriceList = this.priceListService.getPriceList(this.low);
    ...
    useAllButton() {
        if (!this.isAllButton) {
            ...
            this.marketVarietyPropertiesSelected = [];
            this.subscriptions.push(this.priceListService.getMarketVarietyPropertyAll(this.marketId, this.typeOfFlowersId, this.shippingPolicy.id, this.low)
                .subscribe((res) => {
    ...
    getPriceLists() {
        if (!this.isAllTypeOfFlowers) {
            if (this.isAllButton) {
                this.subscriptions.push(this.priceListService.getPriceListFile(this.marketVarietyPropertiesSelectedAll, this.low, this.extension)
                    .subscribe((status) => this.isDownloadingFile = status));
            } else {
                this.subscriptions.push(this.priceListService.getMarketVarietyPropertyAll(this.marketId, this.typeOfFlowersId, this.shippingPolicy.id, this.low)
                    .subscribe((res) => {
                        this.subscriptions.push(this.priceListService.getPriceListFile(res, this.low, this.extension)
                            .subscribe((status) => this.isDownloadingFile = status));
                    }));
            }
        } else {
            ...
            this.subscriptions.push(Observable.forkJoin(
                ...this.typeOfFlowersByCompany.map((typeOfFlowers) =>
                    this.priceListService.getMarketVarietyPropertyAll(this.marketId, typeOfFlowers.id, this.shippingPolicy.id, this.low))
            ).subscribe((result: Array<any>) => {
                result.forEach((priceLists) => flowersPriceLists.push(...priceLists));
                this.subscriptions.push(this.priceListService.getPriceListFile(flowersPriceLists, this.low, this.extension)
                    .subscribe((status) => this.isDownloadingFile = status));
            }));
        }
    }
    ...
    save(item: PriceListDTO, event, cellIndex) {
        ...
        if (isUndefined(this.priceList)) {
            ...
            this.priceList.type = PriceListType.LOW;
    ...
    getMarketVarietyProperty(varietyId: number) {
        this.subscriptions.push(this.priceListService.getMarketVarietyProperty(this.marketId, varietyId, this.shippingPolicy.id, this.low).subscribe((res) => {
            this.marketVarietyPropertiesSelectedAll.push(res);
