...
currentAccount: any;
    ...
    transition() {
        this.router.navigate(['/sort'], {queryParams:
            {
                page: this.page,
                size: this.itemsPerPage,
    ...
    trackId(index: number, item: Sort) {
        return item.id;
    }

    registerChangeInSorts() {
        this.eventSubscriber = this.eventManager.subscribe('sortListModification', (response) => this.loadAll());
    }
